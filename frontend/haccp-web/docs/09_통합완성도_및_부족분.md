...
| 스마트일지 UI | 동결 | API 잔존 |
| 감사추출 UI | 동결 | API 잔존 |
| 법적서류 인페이지 미리보기 | 동결 | G-25 · 다운로드만 ([`11` §2.9](11_프레임워크_파일_보안_작성규칙.md)) |
...
| ID | 부족 | 현상 | 권장 |
|----|------|------|------|
| G-10 | ~~CCP/위생/BizOps 검증 밀도~~ | 냉장 대비 스모크·엣지 문서·테스트 부족 | **완료(문서)** — 2026-08-10 STEP 06. [`13_스모크_매트릭스.md`](13_스모크_매트릭스.md) 표1. 실행은 수동 표 + `prod_smoke.sh` |
...
| G-15 | ~~BizOps 다중 base~~ | fac/inv/prc URL은 있는데 작성 UI는 HWP로 이전된 화면 다수 | **완료(문서)** — 2026-08-11 STEP 23. [`08` §5.7](08_HACCP_FE_BE_통합_상세스펙.md)·[`07` §6.4](07_메뉴_화면_API_DB_전수.md). 활성 BizOps UI=`facility-equipment-check`만 · HWP 이관 4종 `bizOpsApi` 호출 0. API 삭제는 별도 승인 |
...
| ID | 부족 | 현상 | 권장 |
|----|------|------|------|
| G-20 | ~~FE docs만 상세, BE docs는 06 요약~~ | 통합 리뷰 시 BE 전수는 08에 의존 | **완료** — 2026-08-11 STEP 22. BE [`00_문서인덱스.md`](../../../backend/haccp-api/docs/00_문서인덱스.md)에 FE 08/09/11·런북 미러 링크 |
| G-21 | ~~E2E/자동화 테스트~~ | 핵심 루프 자동 테스트 부재 | **완료(상신 실측)** — 2026-08-11 STEP 28 보강. Playwright `e2e/cold-request.spec.ts` 2 passed(0 skipped): 로그인→냉장→목록 + 신규·저장·상신. Credentials `haccp-write-user`. **main Jenkins 미포함** · `Jenkinsfile.e2e` |
| G-22 | ~~멀티탭 로그아웃~~ | MES F174 대비 Path basename 갭·문서 부족 | **완료** — 2026-08-11 STEP 24. `authPaths`로 `/haccp/login` 정합 · [`08` §3.4](08_HACCP_FE_BE_통합_상세스펙.md)·[`04` §5.1](04_인증_보안_JWT.md) · vitest `authPaths`/`authCrossTab` |
| G-23 | ~~그리드 pref·가상화~~ | env는 있으나 화면별 편차·PAGE_SIZE 미사용 혼동 | **완료** — 2026-08-11 STEP 25. [`08` §3.5](08_HACCP_FE_BE_통합_상세스펙.md)·스모크 §5.5. 활성=VIRTUAL_THRESHOLD · PAGE_SIZE는 예약. 문서함·감사로그·설비이력 경로 OK · 크리티컬 0 |
| G-24 | ~~서명 UX~~ | 클립보드·업로드 경로 환경 의존 | **완료(문서)** — 2026-08-11 STEP 26. [`11` §2.8](11_프레임워크_파일_보안_작성규칙.md) 등록·사용·실패 시 안내. 코드 변경 없음 |
| G-25 | ~~법적서류 PDF/HWP 혼재~~ | 미리보기 제거됨 · 다운로드만 | **동결(의도)** — 2026-08-11 STEP 27. [`11` §2.9](11_프레임워크_파일_보안_작성규칙.md). 인페이지 미리보기 재도입은 별도 제품 STEP · 코드 변경 없음 |
...
| 시설점검 | 부분 | BizOps FACILITY만 DB | ~~G-15~~(문서) · 스모크는 G-10 매트릭스 |
| 건강진단 | 완료 | 그리드·첨부 | — |
| 설비이력 | 완료 | M-D·사진 | — |
...
4. ~~G-10 / G-11 스모크 매트릭스~~ — 완료(문서) (2026-08-10, STEP 06, [`13_스모크_매트릭스.md`](13_스모크_매트릭스.md))  
