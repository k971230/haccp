...
> 정본: `frontend/haccp-web/docs/09_통합완성도_및_부족분.md`  
> 작성일: 2026-08-10 · 개발자: 박승우  
> 목적: 나중에 docs를 **통합 리뷰**할 때 “뭐가 됐고 / 뭐가 부족한지” 판정 근거.  
> 상세 표·URL: [`07`](07_메뉴_화면_API_DB_전수.md) · [`08`](08_HACCP_FE_BE_통합_상세스펙.md) · 요약 갭: [`06`](06_업무_CRUD.md) · 소스 지도: [`10`](10_파일구조_컴포넌트_함수지도.md) · 프레임워크·파일·보안: [`11`](11_프레임워크_파일_보안_작성규칙.md) · 운영·인증: [`01`](01_운영규칙.md)·[`04`](04_인증_보안_JWT.md)
...
| 시스템 관리 CRUD·이력 | 완료 | SystemManagementPage · sys API |
...
| ID | 부족 | 현상 | 권장 |
|----|------|------|------|
| G-01 | `approval-history` 메뉴 | FE·API 있는데 DEMO `tbl_menu` 활성 leaf 없을 수 있음 | migrate/메뉴관리로 MAPR leaf 복구 · 07§4.8 |
| G-02 | UV/PV 통계 운영 의존 | Job cron·타임존·기동 옵션 미설정 시 표 공백 | `.env` VIEW_STAT_* 확인 · 배포 런북에 일집계 명시 |
| G-03 | migrate `47` 파일 2개 | `47_migrate_check_item_admin_crud` · `47_migrate_legal_upload_type` 번호 충돌 | 적용 순서 문서화 또는 48로 재번호 (리뷰 안건) |
| G-04 | HWP/PDF 환경 | `APP_RHWP_CLI_PATH`·폰트·볼륨 없으면 export-pdf·미리보기 실패 | 배포 체크리스트 · 로컬 스모크 시나리오 |
| G-05 | 양식 form_path 공백 | LAW 등 유형만 있고 파일 없으면 form GET 400 (FE 가드는 됨) | 운영 가이드: 템플릿 저장 후 다운로드 |

### 3.2 P1 — 기능은 되나 완성도·정리 부족

| ID | 부족 | 현상 | 권장 |
|----|------|------|------|
| G-10 | CCP/위생/BizOps 검증 밀도 | 냉장 대비 스모크·엣지(결재중 수정 차단 등) 문서·테스트 부족 | 양식별 스모크 표 추가 (냉장 골드 복제) |
| G-11 | HWP 27 leaf 균일 검증 | 공용 Editor만 믿고 leaf별 권한·tmpl·시드 샘플 편차 | DEMO 시드(41) 대비 leaf 샘플 존재 여부 점검표 |
| G-12 | 오늘 할 일 Job 의존 | 00:05 Job·로그인 보정 없으면 과제 공백 | 기동/로그인 generate 경로 문서화 |
| G-13 | dead API/코드 | `ccpMetalApi`·`ccpVerificationApi` · MasterData equipment/pest config | 삭제 또는 deprecated 주석 Wave |
| G-14 | 잔존 API (동결 UI) | smart-diary-* · audit-export | 동결 유지 or API 폐기 결정 |
| G-15 | BizOps 다중 base | fac/inv/prc URL은 있는데 작성 UI는 HWP로 이전된 화면 다수 | 08에 “API 잔존·UI HWP” 표기 유지 · 혼동 방지 |
| G-16 | 문서 06 vs 09 | 06의 완료표와 09가 일부 중복·시점 차이 | 합본 시 09를 판정 정본으로 |

### 3.3 P2 — 품질·운영·문서 부채

| ID | 부족 | 현상 | 권장 |
|----|------|------|------|
| G-20 | FE docs만 상세, BE docs는 06 요약 | 통합 리뷰 시 BE 전수는 08에 의존 | 합본 후 BE docs에 08 링크만 유지해도 됨 |
| G-21 | E2E/자동화 테스트 | 핵심 루프 자동 테스트 부재 | Playwright 스모크 (로그인→작성→상신) 후보 |
| G-22 | 멀티탭 로그아웃 | MES F174 규약 대비 HACCP 구현 깊이 문서 부족 | authCrossTab 동작 확인·08 보강 |
| G-23 | 그리드 pref·가상화 | env는 있으나 화면별 편차 | 대량 화면 스모크 |
| G-24 | 서명 UX | 클립보드·업로드 경로 환경 의존 | 사용자 가이드 한 절 |
| G-25 | 법적서류 PDF/HWP 혼재 | 미리보기 제거됨 · 다운로드만 | 의도 확인 (현행 OK면 동결로 표기) |
...
| 결재이력 | 부족 | FE mode=history | G-01 메뉴 |
| 법적서류 | 완료 | M-D·다운로드 | G-05 · G-25 |
...
| Docker/Nginx | 문서 분산 | 30-docker 규칙 | HACCP docs에 배포 절 요약 부족(P2) |
...
| 구분 | 내용 | 조치 후보 |
|------|------|-----------|
| Dead FE path | MasterDataPage 내 equipment/pest config | 삭제 Wave |
| Dead/중복 FE api | ccpMetalApi · ccpVerificationApi | ccpFormsApi로 통일 후 제거 |
| API 잔존 UI 없음 | smart-diary · audit-export | 동결 유지 or 제거 |
| menu_cd ≠ scrn_cd | MFRM equipment-management → equipment-history | 문서화됨 · 리네임은 선택 |
| 셸 CRUD vs 패널 CRUD | 법적서류 search만 | 의도 — 09에 명시 유지 |
| BE docs 얇음 | haccp-api/docs는 06만 | 08·09 링크 (이미 일부) |
...
1. G-01 approval-history 메뉴 복구 여부 결정·적용  
2. G-03 migrate 47 이중 번호 정리안  
3. G-02 / G-04 운영 env 체크리스트를 08 또는 배포 문서에 고정  

### Should (P1)

4. G-10 냉장 외 작성 화면 스모크 표  
5. G-13 dead code 정리 Wave  
6. G-14 잔존 API 폐기/유지 결정  

### Could (P2)

7. G-21 E2E 스모크  
8. 06 내용을 09로 흡수하고 06은 “요약 1페이지”로 축소  
9. BE docs에 08/09 미러 인덱스만 추가  
