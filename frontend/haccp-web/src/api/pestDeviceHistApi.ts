...
/** 방충설비 이력 행 — 그리드·저장 SP camelCase 계약 */
export interface PestDeviceHistRow {
  idx?: number | null;
  pestIdx?: number | null;
  histDt?: string | null;
 ...
 */
export async function savePestDeviceHist(
  // 저장 행 배열 — UI 단건이어도 배열
 ...
 */
export async function validateDeletePestDeviceHist(
  // 삭제 키 객체 배열 — 단건도 [{ idx }]
  keys: { idx: number }[]
): Promise<void> {
 ...
 *   3) 성공 시 반환값 없이 끝나며 화면이 목록을 다시 조회한다
 */
export async function deletePestDeviceHist(
