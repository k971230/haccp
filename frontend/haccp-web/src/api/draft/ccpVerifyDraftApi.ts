/**
 * ccpVerifyDraftApi — CCP 검증점검 양식 작성 API.
 *
 * 개발자: 박승우
 * 일자: 2026-08-24
 * 코멘트:
 ...
 *
 * PIPELINE[HF175] CCP 검증점검 작성 API
 */
 ...
 */
export async function listCcpVerifyDraftForms(): Promise<HtmlFormDraftForm[]> {
 ...
 */
export async function listCcpVerifyDraft(
  // params: 서버 검색 조건 5개
 ...
 */
export async function getCcpVerifyDraftDetail(
 ...
 */
export async function saveCcpVerifyDraft(body: HtmlFormDraftSaveRequest): Promise<number> {
 ...
 */
export async function validateDeleteCcpVerifyDraft(keys: { docIdx: number }[]): Promise<void> {
 ...
 */
export async function deleteCcpVerifyDraft(keys: { docIdx: number }[]): Promise<void> {
  await http.post(`${BASE}/delete`, keys);
}

/** 공통 작성 화면에 넘길 API 묶음 — HtmlFormDraftPage 가 이것만 본다 */
export const ccpVerifyDraftApi: HtmlFormDraftApi = {
  listForms: listCcpVerifyDraftForms,
  list: listCcpVerifyDraft,
  detail: getCcpVerifyDraftDetail,
  save: saveCcpVerifyDraft,
  validateDelete: validateDeleteCcpVerifyDraft,
  remove: deleteCcpVerifyDraft,
};
