...
| 전송 | `REQ` | 불가 | 불가 | — | 가능 |
| 결재완료 | `APV` | 불가 | 불가 | — | 불가 (결재 쪽에서 취소) |

좌측 목록 「제목」은 `tbl_document.title` 이다. 결재 첨부 화면의 비고(`tbl_document.remark`)와 다르다.
전송대기·반려만 고친다. 검색 칸도 제목을 본다. 전송대기 저장은 본문 SP 한 번에 제목을 같이 쓴다.
전송·결재완료는 본문·제목 모두 잠긴다. SP `sp_tbl_document_title_u_000` 이 REQ/APV 를 막는다.
