...
| `hwpOpenMode.ts` | 작성본 / 양식 원본 / 첨부 대기 판정 |
...
- **열기** — 첨부 중 가장 나중 `HWP_SRC` 를 내려받아 편집기에 싣는다. 저장된 문서인데 첨부가 아직 없으면 양식 원본을 열지 않고 기다린다. 원본은 `docIdx` 없는 신규만.
...
저장으로 첨부가 바뀌면 표식이 달라져 다시 연다. `docIdx` 있는데 `HWP_SRC` 가 없으면 양식 원본을
열지 않는다(`hwpOpenMode`). `loadFile` 은 한 줄로 직렬하고, 끝난 뒤 표식이 바뀌었으면 지금 표식을 다시 연다.
...
- 전송 이후 행은 `htmlFormDraftGridRules.isRowEditLocked` 가 제목까지 잠근다
