...
type Row = WorkflowRow & { _key?: string; srcDocNo?: string; caNo?: string; occurDt?: string; deviationDesc?: string };

const FIELD_LABELS: { key: string; label: string; type?: "text" | "date" }[] = [
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 p-3">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_22rem]">
        <section className="flex min-h-0 flex-col overflow-hidden rounded border border-slate-200 bg-white p-2">
          ...
          <MesEditableGrid
            // 열 설정 저장 키
            persistId="doc-corrective-actions"
            // 개선조치 목록
          ...
          {FIELD_LABELS.map((field) => (
            <label key={field.key} className="mb-2 flex flex-col gap-1 text-xs text-slate-600">
              ...
              <Input
                ...
                value={
                  field.type === "date"
                    ? toInput(String(editor?.[field.key] ?? ""))
                    : String(editor?.[field.key] ?? "")
                }
                onChange={(event) =>
                  patch(
                    field.key,
                    field.type === "date" ? fromInput(event.target.value) : event.target.value,
                  )
                }
              />
            </label>
