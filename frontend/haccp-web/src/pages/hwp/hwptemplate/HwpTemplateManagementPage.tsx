...
export default function HwpTemplateManagementPage() {
  ...
  const useOpts = useMemo(
    ...
    ]).map((code) => ({ value: String(code.subCd).toUpperCase(), label: code.codeNm })),
    [useCodes.codes],
  );
  ...
  const formUrlOf = (row: EditableRow<TmplListRow> | null): string | null => {
    ...
    return `/api/v1/doc/templates/${encodeURIComponent(String(row.tmplCd))}/form`;
  };
   ...
   *   3) draft(C)·파일 미등록은 안내만 하고 요청하지 않는다
   */
