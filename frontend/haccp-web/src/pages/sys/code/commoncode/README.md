...
조회는 회사코드 완전 격리다(`WHERE c.co_cd = p_co_cd`). 표준코드 `0000`을 상속받지 않으므로 신규 업체는 `03_code_seed` + `05_form_seed` + `06_company_seed` + `07_company_forms` 네 본이 표준코드를 복제해 넣는다 (`sp_tbl_company_init_c_000` 은 없다). 기존 업체분은 `db_sasshaccp/02_seed.sql`로 백필했다.
