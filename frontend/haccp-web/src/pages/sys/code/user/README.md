...
- 필수는 사용자 ID·사용자명·권한그룹. 부서는 `tbl_user.dept_cd`가 NULL 허용이라 비워 둘 수 있고, 룩업에 `(없음)` 행이 있다
- 비밀번호는 **값을 입력했을 때만** 서버에서 BCrypt로 다시 해싱한다. 빈 값이면 기존 비밀번호 유지
- `empCd`·`posCd`는 이 화면에서 다루지 않으며 저장 payload에서 제거된다
- **`lockYn`(잠금)은 이 화면이 다룬다.** 연속 로그인 실패가 임계를 넘으면 서버가 `Y` 로 잠그고
  (`sp_tbl_user_login_u_000`) 로그인이 거절된다. **푸는 화면은 여기 하나다** — `N` 으로 저장하면
  SP 가 실패횟수까지 0 으로 되돌린다. 예전에는 payload 에서 빼서 DB 를 직접 고치는 수밖에 없었다.
  이 칸만 서버가 스스로 바꾸므로, 목록이 낡은 사이 그 행을 저장하면 잠금이 같이 풀릴 수 있다
