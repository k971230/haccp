...
export interface CcpGenericMonitorPageProps {
  // 권한·API 화면코드 — 가열·살균·여과 3개만. URL 과 같다
  screenCode?: CcpGenericScrnCd;
  // 메뉴에서 양식을 고정할 때
...
export default function CcpGenericMonitorPage({
  ...
  const loadTemplates = useCallback(async () => {
    try {
      const next = await listGenericCcpTemplates(screenCode);
  ...
  const handleSelect = useCallback(async (key: string | null) => {
    await selectKey(key, async (k, row) => {
      ...
      try {
        const detail = await getGenericCcpDetail(screenCode, row.docIdx);
  ...
  }, [defaultTmplCd, getBuffer, pageTmplCd, resolveTmpl, screenCode, selectKey, selected?.tmplCd, user?.userId, user?.userNm]);
    ...
    try {
      const err = await saveAll({
        ...
        saveOne: async (_row, b) => {
          ...
          const saved = await saveGenericCcpMonitor(screenCode, {
            docIdx: b.docIdx,
            baseDt: b.baseKey,
            tmplCd: b.tmplCd,
  ...
  const handleDelete = () => asyncAct.run(async () => {
    ...
    try {
      const keys = [{ docIdx }];
      await validateDeleteGenericCcp(screenCode, keys);
      if (!(await mesConfirmDanger("선택한 공통 CCP 일지를 삭제하시겠습니까?"))) return;
      await deleteGenericCcp(screenCode, keys);
      ...
      {docIdx ? (
        <DocumentApprovalToolbar
          ...
          onApproved={() => {
            void loadList();
            if (docIdx && activeKey) {
              void getGenericCcpDetail(screenCode, docIdx).then((d) => {
