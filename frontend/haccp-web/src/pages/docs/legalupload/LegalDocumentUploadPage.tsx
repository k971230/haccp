...
export default function LegalDocumentUploadPage({
  ...
  const tmplCols = useMemo(
    () => buildTmplColumns(canWrite || canModify, { onPickTmplFile: pickTmplFile }),
    // eslint-disable-next-line react-hooks/exhaustive-deps -- pick는 권한만 봄
    [canModify, canWrite],
  );

  const docCols = useMemo(
    () => buildDocColumns(canWrite || canModify, { onPickDocFile: pickDocFile }),
    // eslint-disable-next-line react-hooks/exhaustive-deps -- pick는 권한만 봄
    [canModify, canWrite],
  );
