...
import { useDocFormSession } from "@/hooks/useDocFormSession";
...
import {
  ...
  buildListColumns,
  fileSize,
  flattenSignToWhitePng,
  listPersistIdOf,
  parseHwpSrcSeq,
  type Buf,
  type ListMeta,
} from "./HwpDocumentEditorRule";
...
export default function HwpDocumentEditorPage({
  ...
  const screenCode = screenCodeProp || pageScrnCd || FALLBACK_SCRN_CD;
  const canWrite = useAuthStore((state) => state.can(screenCode, "write") || state.can(FALLBACK_SCRN_CD, "write"));
  const canModify = useAuthStore((state) => state.can(screenCode, "modify") || state.can(FALLBACK_SCRN_CD, "modify"));
  const canDelete = useAuthStore((state) => state.can(screenCode, "delete") || state.can(FALLBACK_SCRN_CD, "delete"));
  ...
  const canPrint = useAuthStore((state) => state.can(screenCode, "print") || state.can(FALLBACK_SCRN_CD, "print"));
    ...
    <DocFormLayout>
      ...
      <DocFormBody withSummary>
        <DocFormDocumentList label="문서 목록">
          <MesEditableGrid
            // 열 설정 저장 키 — 일자 문서 목록
            persistId={listPersistIdOf(screenCode)}
            // 서버 문서 + draft
