...
} from "@/api/docs/docCycleApi";
// 역할 — 결재선 후보 — 결재선관리와 같은 목록. 작성 단계 담당자·부서까지 온다
import { listApprovalLines, type ApprovalLine } from "@/api/sys/approvalLineApi";
...
export default function ScheduleCycleManagementPage() {
  ...
  // 결재선 후보 — 결재선관리와 동일 목록. 사용중만 룩업에 올린다
  const [apprLines, setApprLines] = useState<ApprovalLine[]>([]);
  ...
  const loadCycle = useCallback(async (tmplCd: string | null) => {
    ...
    try {
      ...
      const listed = formsRef.current.find((row) => row.tmplCd === tmplCd);
      const base = emptyForm();
      // 주기 없을 때(= 신규) 사용양식에 이미 붙은 결재선을 보여 준다
      if (!rule) {
        base.apprLineCd = String(listed?.apprLineCd ?? "");
        base.apprLineNm = String(listed?.apprLineNm ?? "");
      }
      const next = rule
        ? detailsToForm({
          ...base,
          ...
          apprLineCd: String(rule.apprLineCd ?? listed?.apprLineCd ?? ""),
          apprLineNm: String(rule.apprLineNm ?? listed?.apprLineNm ?? ""),
  ...
  // 결재선 후보 — 화면 진입 시 1회. 결재선관리와 같은 API
  useEffect(() => {
    void (async () => {
      try {
        const rows = await listApprovalLines();
        setApprLines(rows.filter((row) => String(row.apprLineCd ?? "")));
      } catch (error) {
        mesError(error);
      }
    })();
   ...
   * 일자: 2026-08-20
   * 코멘트:
   *   1) 결재선 선택 팝업 — 결재선관리와 같은 목록. 사용중만 보여 준다
   *   2) 결재선 「선택」 버튼에서 호출한다
   *   3) 작성(WRITE)에 사람이 있으면 담당자·담당부서를 채운다. (없음)은 결재선만 비우고 담당자는 그대로 둔다
   */
  const openApprLineLookup = () => {
    if (!canEdit) return;
    const usable = apprLines.filter((row) => String(row.useYn ?? "Y").toUpperCase() !== "N");
    openModal("CodeLookup", {
      title: "결재선 선택",
      scrnCd: SCRN_CD,
      options: usable.map((row) => ({
        value: row.apprLineCd,
        // 이름 (코드) — 코드명 칸에서 코드로도 조회된다
        label: row.apprLineNm ? `${row.apprLineNm} (${row.apprLineCd})` : row.apprLineCd,
      })),
      value: form.apprLineCd,
      allowEmpty: true,
      onSelect: (code) => {
        // 빈 코드일 때(= 없음) 결재선만 지운다. 담당자·부서는 손대지 않는다
        if (!code) {
          setForm((prev) => ({
            ...prev,
            apprLineCd: "",
        ...
        const picked = usable.find((row) => row.apprLineCd === code);
        // 작성 단계에 사람이 있을 때(= 결재선관리 작성자) 담당자·부서로 넣는다
        const write = picked?.steps?.find((step) =>
          String(step.roleCd ?? "").toUpperCase() === "WRITE"
        ...
        setForm((prev) => {
          const next = {
            ...prev,
            apprLineCd: code,
            apprLineNm: picked?.apprLineNm ?? "",
          };
          if (!write) return next;
          const userId = String(write.approverId ?? "");
          const fromUser = userRows.find((row) => row.userId === userId);
          return {
            ...next,
            userId,
            userNm: String(write.approverNm ?? fromUser?.userNm ?? userId),
            // 단계 부서가 비면 사용자 소속으로 채운다
            deptCd: String(write.deptCd ?? fromUser?.deptCd ?? ""),
            deptNm: String(write.deptNm ?? fromUser?.deptNm ?? ""),
          };
        });
      },
    });
  ...
  const handleSave = async () => {
    ...
    try {
      await saveDocCycle({
        ...
        apprLineCd: form.apprLineCd || null,
        useYn: form.useYn,
            ...
            <div className={splitPanelClass}>
              <div className={gridHeadClass}>
                <div className="flex min-w-0 items-center gap-2 overflow-hidden">
                  ...
                  {activeForm ? (
                    <SysYnBadge
                      // 주기 대상 구분 — sys/usr. 문구는 공통코드 sys-yn
                ...
                <div className="min-h-0 flex-1 overflow-auto p-4">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div className={`${formLabelClass} md:col-span-2`}>
                      {/* 결재선 — 결재선관리 목록. 작성자에 사람이 있으면 담당자·부서를 채운다 */}
                      결재선
                      <div className="flex min-w-0 flex-nowrap items-stretch gap-1">
                        <input
                          className={`${formFieldClass} min-w-0 flex-1`}
                          value={form.apprLineNm || form.apprLineCd}
                          readOnly
                          placeholder="선택하세요"
                        />
                        <MesButton
                          // 결재선 룩업 — 담당자 선택과 같은 한 줄 입력+버튼
                          variant="search"
                          size="sm"
                          className="shrink-0"
                          disabled={!canEdit}
                          onClick={openApprLineLookup}
                        >
                          선택
                        </MesButton>
                      </div>
