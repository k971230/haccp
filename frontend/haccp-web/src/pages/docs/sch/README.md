...
- 반복설정은 주기 콤보에 따라 영역만 교체 — 매일·비정기 안내 / 매주 요일 / 매월 실행일(파랑)·말일 실행(노랑, 전폭) / 분기·반기·매년 월+일·말일 실행
- 매일 안내 문구의 비영업일은 주말·공휴일. 날짜 정본은 BE `holidays-kr.json`
...
|---|---|---|---|
| 양식 목록 | `listDocCycleForms` | `sp_schedule_cycle_management_form_r_000` | `tbl_company_template` `tbl_template` `tbl_schedule_rule` — HTML 은 사용 중인 지면 버전이 있는 것만 |
