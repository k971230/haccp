...
- 좌측 버튼: 행추가 · 삭제 · 저장. 행추가는 표준(`tml_ccp_pkg_000`) 복사 pending. 좌 저장이 `copy` INSERT·양식명 `name`.
- 표준(`tml_ccp_pkg_000`)은 `tbl_check_item` 가상행(한계기준 2 · 주기 · 방법 · 개선조치). 양식명·항목 수정·삭제 불가.
- 자사 저장은 `tml_ccp_pkg_001`부터 채번, 테이블 `tbl_tml_ccp_pkg_ver`. 문서주기 좌측에 오른다.
...
scrnCd `ccp-pkg-template` · API `/api/v1/docs/html/ccp-pkg-template`
