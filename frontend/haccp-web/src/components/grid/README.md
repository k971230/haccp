...
**되돌리는 길은 「열」메뉴 맨 아래 「열 초기화」다** (`useMesTable.resetLayout`).
숨김은 메뉴 체크로, 너비는 끌어서 되돌릴 수 있지만 **순서는 한 칸씩 끌어야** 해서
열을 여럿 옮겨 놓으면 빠져나올 길이 없었다. 셋을 한 번에 기본값으로 돌린다 —
컬럼 정의가 `defaultHidden` 이라고 한 열은 숨긴 채로 남는다.
