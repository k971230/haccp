/**
 * SysYnBadge — 공통코드 sys-yn(시스템제공 / 사용자추가) 헤더 배지.
 *
 * 개발자: 박승우
 * 일자: 2026-08-19
 * 코멘트:
 ...
 *   3) 문구는 공통코드 관리 sys-yn, 판정은 isCompanyForm. src-ty 와 섞지 않는다
 *
 * PIPELINE[HF102] 공통코드 훅
 * PIPELINE[HF123] 사용양식 구분
 */
// 역할 — sys-yn 조회·사용자추가 판정 (공통코드 훅 정본)
 ...
 */
export function SysYnBadge({
  // 서버 구분 값 — sys/usr 또는 레거시 Y/N. 문서주기는 formTy 를 그대로 넘긴다
  sysYn,
}: {
  sysYn?: string | null;
}) {
  // 문구는 공통코드 관리 sys-yn — 화면이 따로 라벨을 두지 않는다
  const { codeMap } = useCommonCodes(SYS_YN_MAIN_CD);
