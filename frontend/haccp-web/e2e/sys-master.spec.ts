...
test.describe("부서관리", () => {
  ...
  test("부서 등록 → DB 반영 → 수정", async ({ page }) => {
    ...
    expect(dbOne(`SELECT co_cd FROM tbl_dept WHERE dept_cd='${DEPT}'`)).toBe(loginCoCd());
  });
  ...
  test("메뉴명을 고치면 DB 에 남는다", async ({ page }) => {
    ...
    dbOne(
      `UPDATE tbl_menu SET menu_nm='오늘 할 일'
        WHERE menu_cd='today-tasks' AND co_cd='${sqlLit(loginCoCd())}' AND menu_nm='${NEW_NM}'`,
    );
    const before = dbOne(`SELECT menu_nm FROM tbl_menu WHERE menu_cd='today-tasks' AND co_cd='${sqlLit(loginCoCd())}'`);
    ...
    expect(dbOne(`SELECT menu_nm FROM tbl_menu WHERE menu_cd='today-tasks' AND co_cd='${sqlLit(loginCoCd())}'`)).toBe(
      NEW_NM,
    );

    // 원래 이름으로 되돌린다 — 다음 시험·사람이 쓰는 화면이다
    dbOne(`UPDATE tbl_menu SET menu_nm='${before}' WHERE menu_cd='today-tasks' AND co_cd='${sqlLit(loginCoCd())}'`);
  });
});
