...
| **23** | E / G-15 | BizOps 다중 base 문서·혼동 방지 | 20 후 | — |
| **24** | E / G-22 | 멀티탭 로그아웃 확인·문서 | 19 후 | — |
...
1. 런북 §12.1 Credentials 6종 체크리스트를 로컬 작업 노트(커밋 금지 경로)에 채울 빈 표를 만든다.
   - haccp-registry-cred : GHCR PAT — classic 권장 스코프 write:packages, read:packages (+ repo)
...
2. Git Credential 의 gho_ 토큰은 write:packages 가 없음을 문서에 경고로 남긴다 (이미 실측됨).
   → 별도 PAT 발급이 필수임을 수용조건에 명시.
...
5. frontend/haccp-web/docs/13_스모크_매트릭스.md
   - 샘플 HWP 5종 전 컬럼
...
작업:
1. 잔존 엔드포인트·FE 참조 전수 grep
2. 결정안 표: 유지(동결+문서) / 폐기(삭제 Wave)
3. 사용자 승인 후 코드·문서 반영 (승인 전 삭제 금지)
4. 09 G-14 상태 갱신

수용조건: 결정 문서화 + (승인 시) 코드/문서 일치 · tsc/compile 통과
[프롬프트 끝]
```
