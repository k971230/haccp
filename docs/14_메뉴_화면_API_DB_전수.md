...
> 작성일: 2026-08-10 · 최종 갱신: 2026-08-26 · 개발자: 박승우  
> **2026-08-26 정리** — 28화면 체계에 맞춰 삭제된 화면 62행을 걷어내고 빠져 있던 7화면
> (결재첨부·결재대기·결재완료 · CCP 검증/포장/가열/금속검출 양식관리)을 채웠다.
> 이 표의 화면 목록은 `tbl_menu`(28) 와 1:1 이다.  
> 범위: HACCP SaaS만 (MES 제외). CRUD 패턴은 [`06_업무_CRUD.md`](13_업무_CRUD_BE.md).  
> 0000(데모식품) 테넌트 기준. 신규 업체 개설은 `db_sasshaccp/06_company_seed.sql`.  
...
| 부모 | menu_cd | 메뉴명 | scrn_cd | sort | module | tmplCd | FE Page | 컴포넌트 | FE API | BE Controller | API | Mapper | SP | Cmds |
|------|---------|--------|---------|------|--------|--------|---------|----------|--------|---------------|-----|--------|----|------|
| `appr` | `attach` | 결재첨부 | `attach` | 210 | APR | — | flow/appr/attach/ApprovalAttachPage | MesDataGrid·ApprovalDocumentPreview·DocumentApprovalToolbar | documentApi | DocumentController | /api/v1/docs/documents/{approval-inbox,approval,files} | mapper/docs/documents/DocumentMapper | sp_tbl_document_appr_inbox_r_000 · sp_tbl_document_approval_c_000 | search·transfer·print |
| `appr` | `sign-ready` | 결재대기 | `sign-ready` | 211 | APR | — | flow/appr/ApprovalSignReadyPage | MesDataGrid·ApprovalDocumentPreview·DocumentApprovalToolbar | documentApi | DocumentController | /api/v1/docs/documents/{approval-inbox,approval} | mapper/docs/documents/DocumentMapper | sp_tbl_document_appr_inbox_r_000 · sp_tbl_document_approval_c_000 | search·print |
| `appr` | `sign-ok` | 결재완료 | `sign-ok` | 212 | APR | — | flow/appr/ApprovalSignOkPage | MesDataGrid·ApprovalDocumentPreview | documentApi | DocumentController | /api/v1/docs/documents/{approval-history,approval} | mapper/docs/documents/DocumentMapper | sp_tbl_document_appr_hist_r_000 · sp_tbl_document_approval_u_000 | search·print |
...
| `html` | `ccp-verify-template` | CCP 검증점검표 양식관리 | `ccp-verify-template` | 312 | FRM | html_sys_006 (표준 원본) | docs/html-form/HtmlFormTemplatePage + ccpverifytemplate | ResizableSplit·MesEditableGrid·CcpVerifyPaper | htmlFormApi | CcpVerifyTemplateController | /api/v1/docs/html-form/ccp-verify-template/* | mapper/docs/htmlform/ccpverifytemplate/CcpVerifyTemplateMapper | sp_tbl_tml_ccp_chk_ver_* | add·save·del·search |
| `html` | `ccp-pkg-template` | CCP 포장공정 일지관리 | `ccp-pkg-template` | 313 | FRM | tml_ccp_pkg_000 (표준 원본) | docs/html-form/HtmlFormTemplatePage + ccppkgtemplate | ResizableSplit·MesEditableGrid·CcpPkgPaper | htmlFormApi | CcpPkgTemplateController | /api/v1/docs/html-form/ccp-pkg-template/* | mapper/docs/htmlform/ccppkgtemplate/CcpPkgTemplateMapper | sp_tbl_tml_ccp_pkg_ver_* | add·save·del·search |
| `html` | `ccp-htg-template` | CCP 가열공정 일지관리 | `ccp-htg-template` | 314 | FRM | tml_ccp_htg_000 (표준 원본) | docs/html-form/HtmlFormTemplatePage + ccphtgtemplate | ResizableSplit·MesEditableGrid·CcpHtgPaper | htmlFormApi | CcpHtgTemplateController | /api/v1/docs/html-form/ccp-htg-template/* | mapper/docs/htmlform/ccphtgtemplate/CcpHtgTemplateMapper | sp_tbl_tml_ccp_htg_ver_* | add·save·del·search |
| `html` | `ccp-mtl-template` | CCP 금속검출공정 일지관리 | `ccp-mtl-template` | 315 | FRM | tml_ccp_mtl_000 (표준 원본) | docs/html-form/HtmlFormTemplatePage + ccpmtltemplate | ResizableSplit·MesEditableGrid·CcpMtlPaper | htmlFormApi | CcpMtlTemplateController | /api/v1/docs/html-form/ccp-mtl-template/* | mapper/docs/htmlform/ccpmtltemplate/CcpMtlTemplateMapper | sp_tbl_tml_ccp_mtl_ver_* | add·save·del·search |
...
| `hwp-doc` | `hwp-write` | HWP 양식 작성 | `hwp-write` | 4401 | DOC | 사용여부 예인 HWP 양식 전체 | draft/HtmlFormDraftPage + draft/hwp-doc/HwpDraftPage·HwpEditorPane·Rule | 좌측 공통 동일 · 우측 rhwp 편집기(renderDetail) · HwpTaskLookupModal | hwpDraftApi·documentApi(templates·files·approval) | HwpDraftController (com.haccp.draft.hwpdoc) | /api/v1/draft/hwp-doc/hwp-write/{forms,list,tasks,detail,save,validate-delete,delete} · POST /api/v1/docs/documents/{docIdx}/files · PUT /api/v1/docs/documents/approval | mapper/draft/hwpdoc/HwpDraftMapper | sp_draft_hwp_r_000 · sp_draft_hwp_task_r_000 · sp_tbl_hwp_document_c_000 · sp_tbl_document_d_000 | add·save·del·search·print·transfer |
...
| com.haccp.docs.document/DocumentMapper | resources/mapper/docs/documents/DocumentMapper.xml |
