...
> 작성일: 2026-08-10 · **스냅샷. 2026-08-21에 재판정하지 않음.** 이후 계층 URL·`com.haccp.docs` 분할·탭 우클릭은 이 표에 없을 수 있다.  
> 개발자: 박승우  
...
| 시스템 관리 CRUD·이력 | 완료 | sys 9화면 폴더(user·department·role·menu·commoncode·approvalline·loginhistory·auditlog·screenusage) · api/sys/* |
...
| 시설점검 DB (html_sys_009) | 부분 | BizOps 연결 |
...
| HWP leaf 1 tmpl : 1 scrn | 화면별 thin Page · `HwpDocumentEditorPage` · migrate 37 |
...
| G-15 | ~~BizOps 다중 base~~ | 폐기·재고·입고·공정 HTML API가 남아 HWP leaf와 혼동 | **완료(코드)** — 2026-08-19. [`08` §5.7](15_HACCP_FE_BE_통합_상세스펙.md)·[`07` §6.4](14_메뉴_화면_API_DB_전수.md). HTML은 시설(`html_sys_009`)·검교정(`html_sys_010`)만. 나머지 4 URL 삭제 |
...
| 시설점검 | 부분 | BizOps html_sys_009/010 HTML | ~~G-15~~(코드 삭제) · 스모크는 G-10 매트릭스 |
| 건강진단 | 완료 | 그리드·첨부 | — |
| 설비이력 | 완료 | M-D·사진 | — |
