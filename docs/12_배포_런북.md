...
원본 HWP는 루트 `docs/templates/`(로컬 전용, git 미추적)에 둔다. 기본값이 그 경로라 `HACCP_TEMPLATE_SRC` 생략 가능하다.
...
| `haccp-smoke-user` | Username/password | prod 스모크 `SMOKE_USER`/`SMOKE_PASS` |
...
| `scripts/db_migrate_dryrun.sh` | CI용 임시 PG 적용 |
...
파이프라인은 Deploy **직전**에 Prod DB migrate를 돌리고, 실패 시 Deploy에 진입하지 않는다.  
CI에서는 `bash scripts/db_migrate_dryrun.sh`로 문법만 검증한다.

## 15. Prod 스모크

자동 최소 9단계는 아래 스크립트. 화면별 수동 표(DB형·HWP 27·CRUD·파일)는  
[`frontend/haccp-web/docs/13_스모크_매트릭스.md`](../frontend/haccp-web/docs/13_스모크_매트릭스.md) (STEP 06).
...
| 7 | cold-monitor list | CCP API·권한 |
...
| api 기동 즉시 사망 | 템플릿 볼륨 빈 상태 | `init_volumes.sh` 후 재기동 |
...
| 27 | `feature/step-27-g25-legal-policy` | G-25 법적서류 미리보기 동결 | - | 진행 중 (2026-08-11) |
| 28 | (잔여) | G-21 Playwright E2E | - | **다음 착수 = STEP 28** ([`13_후속_STEP_16이후.md`](13_후속_STEP_16이후.md)) |
...
| 스모크 매트릭스 | [`frontend/haccp-web/docs/13_스모크_매트릭스.md`](../frontend/haccp-web/docs/13_스모크_매트릭스.md) STEP 19 기록 · HWP 5종 목록 OK · 참조차단 validate-delete 문구 OK |
...
에이전트 프롬프트 정본: [`13_후속_STEP_16이후.md`](13_후속_STEP_16이후.md).
