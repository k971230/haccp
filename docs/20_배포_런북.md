...
| `SCREEN_AUTH_ENFORCE` | `false`면 권한 N 계정도 API가 통과한다(shadow). 실사용 오픈 전 `true` | 스테이징 `true` · 운영 첫날 `false` 후 `true` |
...
메뉴 URL 슬러그 정본은 [`db_sasshaccp/02_seed.sql`](../db_sasshaccp/02_seed.sql)이다. 경로 규칙은 [`24_URL_DB_폴더_패키지_정본.md`](24_URL_DB_폴더_패키지_정본.md).
