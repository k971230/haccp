/**
 * CommonCodeService — 공통코드 관리 화면의 조회·저장·삭제 업무 로직.
 *
 * 개발자: 박승우
 * 일자: 2026-08-12
 * 코멘트:
 *   1) 회사코드·작업자는 요청 본문이 아니라 JWT 컨텍스트에서만 읽어 테넌트 위조를 막는다
 *   2) 저장은 변경 행 수만큼 SP를 반복 호출하며 트랜잭션 하나로 All-or-Nothing이다
 *   3) 삭제는 validate-delete·delete 양쪽에서 같은 assertDeletable을 돌리는 Double Check다
 *
 * PIPELINE[HB94] 공통코드 Service
 */
package com.haccp.sys.commoncode;
...
public class CommonCodeService {
    ...
    /** 삭제 차단 문구에 쓰는 업무명 — "선택한 항목 중 공통코드 '...'" */
    private static final String LABEL = "공통코드";
    /** 감사 이력 대상 테이블명 — audit-target 공통코드 sub_cd와 같아야 화면에 표시명이 붙는다 */
    private static final String AUDIT_TBL = "tbl_code";
    ...
    public List<Map<String, Object>> listGroups(
        ...
        return commonCodeMapper.selectGroups(LoginUserContext.coCd(), text(mainCd), text(codeNm));
    }
    ...
    public List<Map<String, Object>> listDetails(
        ...
        // 관리 화면은 사용중지 코드까지 보여줘야 하므로 useYn은 항상 공백으로 넘긴다
        return commonCodeMapper.selectDetails(LoginUserContext.coCd(), main, text(sysYn), "");
    }
     ...
     * 일자: 2026-08-12
     * 코멘트:
     *   1) 삭제 전 참조 차단 여부만 검사한다 (Double Check 1단계)
     *   2) 화면이 확인 대화상자를 띄우기 직전에 호출한다
     ...
     * 일자: 2026-08-12
     * 코멘트:
     *   1) 참조 차단을 다시 확인하고 통과한 행만 삭제한다 (Double Check 2단계)
    ...
    public void delete(
        ...
        String coCd = LoginUserContext.coCd();
        for (Long idx : idxs) {
            commonCodeMapper.delete(coCd, idx);
