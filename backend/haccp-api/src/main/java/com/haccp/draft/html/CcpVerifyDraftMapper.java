/**
 * CcpVerifyDraftMapper — CCP 검증점검 작성 SP 호출.
 *
 * 개발자: 박승우
 * 일자: 2026-08-24
 * 코멘트:
 *   1) 작성 SP 는 이 화면 전용 sp_ccp_verify_* 다. 기존 sp_tbl_ccp_form_* 은 html_sys_006·hwp_sys_003 전용이라 건드리지 않는다
 *   2) 양식 목록은 양식관리와 같은 sp_tbl_tml_ccp_chk_ver_r_000. 사용여부 Y 만 SQL 이 좁힌다
 *   3) 삭제 차단은 foreach IN 단일 쿼리 (OPS_DELETE)
 *
 * 데이터는 CCP 기존 테이블 tbl_ccp_verify_check / tbl_ccp_verify_item 이다. HYG 테이블을 쓰지 않는다.
 *
 * PIPELINE[HB137] CCP 검증점검 작성 Mapper
 */
...
@Mapper
public interface CcpVerifyDraftMapper {
    ...
    Long save(
            // coCd: JWT 회사코드
            @Param("coCd") String coCd,
            // tmplCd: 작성 양식코드 — tml_ccp_chk_NNN
            @Param("tmplCd") String tmplCd,
