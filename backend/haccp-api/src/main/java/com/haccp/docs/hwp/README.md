...
| POST | `/api/v1/docs/templates/{tmplCd}/form` | `docs.templates.TemplateService.saveForm` | `sp_hwp_template_management_file_c_000` | `tbl_company_template_file` |
| GET | `/api/v1/docs/templates/{tmplCd}/form` | 파일 볼륨 읽기 | (SP 없음) | `HaccpTemplates` / `CustomTemplates` |
| POST | `/api/v1/docs/hwp/hwp-template-management/validate-delete` | `HwpTemplateService.validateDelete` | `sp_tbl_company_template_delete_blocker_r_000` | `tbl_company_template` |
| POST | `/api/v1/docs/hwp/hwp-template-management/delete` | `HwpTemplateService.delete` | `sp_tbl_company_template_d_000` | `tbl_company_template` |
...
삭제는 2026-08-26 에 이 화면 자기 경로로 옮겼다. 그전 `/api/v1/bas/company-templates/*` 는 서빙하는 컨트롤러가 없어 500 이었다.
파일 업로드 I/O는 `docs.templates.TemplateService.saveForm` 그대로.
