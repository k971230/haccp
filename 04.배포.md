...
> 장애 대응·스테이지 상세는 [`05.운영.md`](05.운영.md)

**Jenkins 는 Build Now 하나만 누르면 된다.** 그 전에 아래를 확인한다.
...
| # | 확인 | 어디서 |
|---|---|---|
| 1-1 | Credentials 가 등록돼 있는가 (정본 [`05.운영.md`](05.운영.md) §2.1) | Jenkins > Credentials |
...
1-3 이 없으면 배포가 시크릿을 못 읽는다. **Build Now 는 이 파일을 안 싣는다** — `scripts/deploy_remote.sh` 는 compose·nginx 만 rsync 하고, `.env.docker` 는 서버 잔류분을 `--env-file` 로 읽는다. 보건증 키를 넣는 위치·방법은 [`05.운영.md`](05.운영.md) §9.1.
...
(`00_alter` 는 멱등, `01_sp` 는 `CREATE OR REPLACE` 라 재실행된다). 이미 깔린 운영·시험은 2026-09-15 에 보건증 표·SP·화면까지 맞췄다.
...
문제가 있으면 [`05.운영.md`](05.운영.md) 의 장애 대응 절.
...
| Job | 파일 | 언제 |
|---|---|---|
| `haccp-verify` | `Jenkinsfile.verify` | PR·main. audit + BE/FE 단위시험 + localhost E2E. 배포 없음. 운영 URL·`E2E_ALLOW_PROD_WRITES` 거절 |
| `haccp-audit` | `Jenkinsfile.audit` | nightly. 같은 audit 스크립트. 실패 시 GitHub 이슈 |
| E2E | `Jenkinsfile.e2e` | 배포 뒤 실측(운영 URL 가능). PR 게이트가 아님. 기본값이 운영이라 verify 와 섞지 않는다 |
...
- 시험: [`docs/6_테스트.md`](docs/6_테스트.md)

## 변경

- 2026-08-26 — 신설. DB 는 파이프라인이 안 건드린다는 것과 신규 업체 개설 절차를 적었다.
- 2026-09-03 — 운영·시험에 결재 SP 개명·REVIEW 잔재 제거를 적용하고 1회성 08·09 파일을 지웠다.
- 2026-09-10 — Job `haccp-verify` (`Jenkinsfile.verify`). PR·main 검증. E2E·배포와 섞지 않는다.
- 2026-09-11 — verify 에 localhost E2E 를 넣었다. 운영 URL 은 `scripts/assert_e2e_pr_safe.sh` 가 거절한다.
