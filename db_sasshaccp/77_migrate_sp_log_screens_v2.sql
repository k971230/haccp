-- ============================================================
--  migrate 77 — 로그 3화면 SP를 화면명 규약으로 신규 작성 (v2)
--
--  개발자: 박승우
--  일자: 2026-08-12
--  코멘트:
...
--    4) 공통코드가 co_cd 완전 고유로 바뀌었으므로 audit-target 표시명도 업체 코드에서 찾는다
--    5) 생성 전용 — 구 sp_tbl_login_history_*·sp_tbl_audit_history_*·sp_tbl_screen_usage_* DROP은 80에서 수행
-- ============================================================

SET search_path TO sasshaccp;

-- ------------------------------------------------------------
-- 1. sp_login_history_r_000 — 로그인 이력 조회
--    실패 로그는 존재하지 않는 아이디로도 남으므로 사용자명은 LEFT JOIN이다
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_login_history_r_000(varchar, varchar, varchar, varchar, varchar);
CREATE FUNCTION sp_login_history_r_000(
...
$$;
COMMENT ON FUNCTION sp_login_history_r_000(varchar, varchar, varchar, varchar, varchar) IS
  '로그인 이력 조회 — 기간·아이디·결과 필터, 최신순';

-- ------------------------------------------------------------
-- 2. sp_audit_log_r_000 — 변경 감사 이력 조회
--    메뉴명은 업체 공통코드 audit-target에서 테이블명으로 역매핑한다(70에서 표준코드가 업체로 복제됨)
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_audit_log_r_000(varchar, varchar, varchar, varchar, varchar, varchar);
CREATE FUNCTION sp_audit_log_r_000(
    -- p_co_cd: JWT 회사코드 — 테넌트 범위. 필수 등가 조건
    p_co_cd     varchar,
    -- p_from_dt: 조회 시작일 YYYYMMDD
    p_from_dt   varchar,
...
$$;
COMMENT ON FUNCTION sp_audit_log_r_000(varchar, varchar, varchar, varchar, varchar, varchar) IS
  '변경 감사 이력 조회 — 기간·메뉴키·행위자·행위 필터, 최신순';

-- ------------------------------------------------------------
-- 3. sp_screen_usage_statistics_r_000 — 화면 이용 통계 조회
--    원시 로그가 아니라 일자 집계(tbl_view_stat_daily)를 읽는다
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_screen_usage_statistics_r_000(varchar, varchar, varchar, varchar);
CREATE FUNCTION sp_screen_usage_statistics_r_000(
    -- p_co_cd: JWT 회사코드 — 테넌트 범위. 필수 등가 조건
    p_co_cd   varchar,
...
$$;
COMMENT ON FUNCTION sp_screen_usage_statistics_r_000(varchar, varchar, varchar, varchar) IS
