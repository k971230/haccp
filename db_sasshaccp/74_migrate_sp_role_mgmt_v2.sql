-- ============================================================
--  migrate 74 — 권한그룹 관리 화면 SP를 화면명 규약으로 신규 작성 (v2)
--
--  개발자: 박승우
--  일자: 2026-08-12
--  코멘트:
...
--    5) 생성 전용 — 구 sp_tbl_role_mgmt_* DROP은 80에서 수행
-- ============================================================

SET search_path TO sasshaccp;

-- ------------------------------------------------------------
-- 1. sp_role_management_r_000 — 권한그룹 목록 (좌측 마스터)
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_role_management_r_000(varchar, varchar, varchar, varchar);
CREATE FUNCTION sp_role_management_r_000(
...
$$;
COMMENT ON FUNCTION sp_role_management_r_000(varchar, varchar, varchar, varchar) IS
  '권한그룹 목록 — 테넌트 범위 + 헤더 코드·명·사용여부 LIKE';

-- ------------------------------------------------------------
-- 2. sp_role_management_c_000 — 권한그룹 저장
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE sp_role_management_c_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd     varchar,
...
BEGIN
    ...
    END IF;
END$$;
COMMENT ON PROCEDURE sp_role_management_c_000(varchar, bigint, varchar, varchar, varchar, varchar, varchar) IS
  '권한그룹 저장 — 신규는 코드 중복 검사, 수정은 명·설명·사용여부만';

-- ------------------------------------------------------------
-- 3. sp_role_management_delete_blocker_r_000 — 삭제 참조 검증
--    이 권한그룹을 쓰는 사용자가 있으면 로그인 권한이 사라지므로 차단한다
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_role_management_delete_blocker_r_000(varchar, bigint[]);
CREATE FUNCTION sp_role_management_delete_blocker_r_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
    -- p_idxs: 삭제 대상 대리키 배열
    p_idxs  bigint[]
...
RETURNS TABLE(ref_key varchar, target varchar) LANGUAGE sql AS $$
    SELECT r.usrgrp_cd::varchar AS ref_key,
     ...
     LIMIT 1;
$$;
COMMENT ON FUNCTION sp_role_management_delete_blocker_r_000(varchar, bigint[]) IS
  '권한그룹 삭제 차단 — 사용 중인 사용자 존재 시 불가. 위반 첫 건만 반환';

-- ------------------------------------------------------------
-- 4. sp_role_management_d_000 — 권한그룹 삭제
--    통과하면 그룹에 딸린 화면권한 행까지 함께 정리한다(남겨도 참조할 주체가 없다)
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE sp_role_management_d_000(
...
END$$;
COMMENT ON PROCEDURE sp_role_management_d_000(varchar, bigint) IS
  '권한그룹 삭제 — 사용자 참조 차단 후 화면권한까지 함께 삭제';

-- ------------------------------------------------------------
-- 5. sp_role_management_screen_r_000 — 권한그룹별 화면 권한
--    권한관리 화면 우측 매트릭스와 로그인 직후 버튼 권한 판정이 함께 쓴다
--    tbl_screen을 기준으로 잡아야 아직 설정하지 않은 화면도 N으로 표시된다
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_role_management_screen_r_000(varchar, varchar);
CREATE FUNCTION sp_role_management_screen_r_000(
...
$$;
COMMENT ON FUNCTION sp_role_management_screen_r_000(varchar, varchar) IS
  '권한그룹별 화면 권한 — 미설정 화면도 N으로 채워 전체 목록 반환';

-- ------------------------------------------------------------
-- 6. sp_role_management_screen_c_000 — 화면 권한 1건 업서트
--    화면은 변경된 행만 모아 이 SP를 반복 호출한다(트랜잭션은 Spring이 잡는다)
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE sp_role_management_screen_c_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd     varchar,
    -- p_usrgrp_cd: 권한그룹코드
    p_usrgrp_cd varchar,
...
BEGIN
    ...
    ON CONFLICT (co_cd, usrgrp_cd, scrn_cd) DO UPDATE SET
        ...
        upd_dt    = now();
END$$;
COMMENT ON PROCEDURE sp_role_management_screen_c_000(varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar) IS
