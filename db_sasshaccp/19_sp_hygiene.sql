...
-- 일자: 2026-08-19
-- 코멘트:
--   1) HTML 일일(html_sys_007)·방충(html_sys_008)을 한 API 계약으로 저장한다
--      개인·작업장·용수 분기는 HWP leaf 호환을 위해 hwp_sys_009/010/021 코드만 맞춘다
    ...
    SELECT d.idx, x.hdr_idx, d.doc_no, x.base_dt, x.base_dt_to, x.checker_nm, d.status,
      ...
      JOIN LATERAL (
          SELECT h.idx hdr_idx, h.base_dt, NULL::varchar base_dt_to, h.checker_nm,
            ...
            FROM tbl_daily_hygiene h WHERE p_tmpl_cd='html_sys_007' AND h.doc_idx=d.idx AND h.co_cd=d.co_cd
          UNION ALL
          SELECT h.idx, h.base_dt, NULL, h.checker_nm,
            ...
            FROM tbl_personal_hygiene h WHERE p_tmpl_cd='hwp_sys_009' AND h.doc_idx=d.idx AND h.co_cd=d.co_cd
          UNION ALL
          SELECT h.idx, h.base_dt, h.base_dt_to, h.checker_nm,
            ...
            FROM tbl_area_hygiene h WHERE p_tmpl_cd='hwp_sys_010' AND h.doc_idx=d.idx AND h.co_cd=d.co_cd
          UNION ALL
          SELECT h.idx, h.base_dt, NULL, h.checker_nm,
            ...
            FROM tbl_pest_check h WHERE p_tmpl_cd='html_sys_008' AND h.doc_idx=d.idx AND h.co_cd=d.co_cd
          UNION ALL
          SELECT h.idx, h.base_dt, h.base_dt_to, NULL,
            ...
            FROM tbl_water_check h WHERE p_tmpl_cd='hwp_sys_021' AND h.doc_idx=d.idx AND h.co_cd=d.co_cd
      ) x ON true
...
BEGIN
  IF coalesce(p_doc_idx,0)=0 THEN
    ...
    IF p_tmpl_cd='html_sys_008' THEN
  ...
  SELECT CASE p_tmpl_cd
    WHEN 'html_sys_007' THEN (SELECT h.idx FROM tbl_daily_hygiene h WHERE h.co_cd=p_co_cd AND h.doc_idx=p_doc_idx)
    WHEN 'hwp_sys_009' THEN (SELECT h.idx FROM tbl_personal_hygiene h WHERE h.co_cd=p_co_cd AND h.doc_idx=p_doc_idx)
    WHEN 'hwp_sys_010' THEN (SELECT h.idx FROM tbl_area_hygiene h WHERE h.co_cd=p_co_cd AND h.doc_idx=p_doc_idx)
    WHEN 'html_sys_008' THEN (SELECT h.idx FROM tbl_pest_check h WHERE h.co_cd=p_co_cd AND h.doc_idx=p_doc_idx)
    WHEN 'hwp_sys_021' THEN (SELECT h.idx FROM tbl_water_check h WHERE h.co_cd=p_co_cd AND h.doc_idx=p_doc_idx)
  END INTO v_hdr;
  ...
  IF p_tmpl_cd='html_sys_007' THEN
    ...
    SELECT jsonb_build_object(
      ...
      LEFT JOIN tbl_check_item ci ON ci.tmpl_cd='html_sys_007' AND ci.item_cd=i.item_cd
     WHERE h.idx=v_hdr AND h.co_cd=p_co_cd
     GROUP BY h.before_time, h.during_time, h.checker_nm;
  ELSIF p_tmpl_cd='hwp_sys_009' THEN
  ...
  ELSIF p_tmpl_cd='hwp_sys_010' THEN
    SELECT jsonb_build_object(
  ...
  ELSIF p_tmpl_cd='html_sys_008' THEN
...
BEGIN
 ...
 IF p_tmpl_cd='html_sys_007' THEN SELECT idx INTO v_hdr FROM tbl_daily_hygiene WHERE co_cd=p_co_cd AND doc_idx=p_doc_idx; DELETE FROM tbl_daily_hygiene_item WHERE co_cd=p_co_cd AND hdr_idx=v_hdr; DELETE FROM tbl_daily_hygiene WHERE co_cd=p_co_cd AND idx=v_hdr;
 ELSIF p_tmpl_cd='hwp_sys_009' THEN SELECT idx INTO v_hdr FROM tbl_personal_hygiene WHERE co_cd=p_co_cd AND doc_idx=p_doc_idx; DELETE FROM tbl_personal_hygiene_row WHERE co_cd=p_co_cd AND hdr_idx=v_hdr; DELETE FROM tbl_personal_hygiene WHERE co_cd=p_co_cd AND idx=v_hdr;
 ELSIF p_tmpl_cd='hwp_sys_010' THEN SELECT idx INTO v_hdr FROM tbl_area_hygiene WHERE co_cd=p_co_cd AND doc_idx=p_doc_idx; DELETE FROM tbl_area_hygiene_result r USING tbl_area_hygiene_item i WHERE r.item_idx=i.idx AND r.co_cd=i.co_cd AND i.co_cd=p_co_cd AND i.hdr_idx=v_hdr; DELETE FROM tbl_area_hygiene_item WHERE co_cd=p_co_cd AND hdr_idx=v_hdr; DELETE FROM tbl_area_hygiene_signer WHERE co_cd=p_co_cd AND hdr_idx=v_hdr; DELETE FROM tbl_area_hygiene WHERE co_cd=p_co_cd AND idx=v_hdr;
 ELSIF p_tmpl_cd='html_sys_008' THEN SELECT idx INTO v_hdr FROM tbl_pest_check WHERE co_cd=p_co_cd AND doc_idx=p_doc_idx; DELETE FROM tbl_pest_check_row WHERE co_cd=p_co_cd AND hdr_idx=v_hdr; DELETE FROM tbl_pest_check WHERE co_cd=p_co_cd AND idx=v_hdr;
