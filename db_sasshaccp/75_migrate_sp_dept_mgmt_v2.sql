-- ============================================================
--  migrate 75 — 부서 관리 화면 SP를 화면명 규약으로 신규 작성 (v2)
--
--  개발자: 박승우
--  일자: 2026-08-12
--  코멘트:
...
--    3) 화면 단위 SP는 sp_{화면명}_{r|c|d|u}_{nnn} 규약을 따른다 — sp_tbl_ 접두를 쓰지 않는다
--    4) 생성 전용 — 구 sp_tbl_dept_mgmt_* DROP은 80에서 수행
-- ============================================================

SET search_path TO sasshaccp;

-- ------------------------------------------------------------
-- 1. sp_department_management_r_000 — 부서 목록 (트리 정렬 · 상위부서명)
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_department_management_r_000(varchar, varchar, varchar, varchar);
CREATE FUNCTION sp_department_management_r_000(
...
$$;
COMMENT ON FUNCTION sp_department_management_r_000(varchar, varchar, varchar, varchar) IS
  '부서 목록 — 상위부서명 self JOIN, 최상위 우선 트리 정렬, 헤더 코드·명·사용여부 LIKE';

-- ------------------------------------------------------------
-- 2. sp_department_management_c_000 — 부서 저장
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE sp_department_management_c_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd     varchar,
...
BEGIN
    ...
    END IF;
END$$;
COMMENT ON PROCEDURE sp_department_management_c_000(varchar, bigint, varchar, varchar, varchar, int, varchar, varchar) IS
  '부서 저장 — 신규는 코드 중복 검사, 수정은 자기참조 방지';

-- ------------------------------------------------------------
-- 3. sp_department_management_delete_blocker_r_000 — 삭제 참조 검증
--    사유 우선순위: 직접 사용자 > 하위트리 사용자 > 직속 하위 부서
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_department_management_delete_blocker_r_000(varchar, bigint[]);
CREATE FUNCTION sp_department_management_delete_blocker_r_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
    -- p_idxs: 삭제 대상 대리키 배열
    p_idxs  bigint[]
...
RETURNS TABLE(ref_key varchar, target varchar) LANGUAGE sql AS $$
    ...
    SELECT t.dept_cd::varchar AS ref_key,
     ...
     LIMIT 1;
$$;
COMMENT ON FUNCTION sp_department_management_delete_blocker_r_000(varchar, bigint[]) IS
  '부서 삭제 차단 — 사용자·하위트리 사용자·하위 부서. 위반 첫 건만 반환';

-- ------------------------------------------------------------
-- 4. sp_department_management_d_000 — 부서 삭제
--    blocker와 같은 세 조건을 삭제 직전에 다시 본다
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE sp_department_management_d_000(
...
END$$;
COMMENT ON PROCEDURE sp_department_management_d_000(varchar, bigint) IS
