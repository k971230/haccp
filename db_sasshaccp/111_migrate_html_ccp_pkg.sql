-- ============================================================
-- 111 — CCP-1B 포장 모니터링일지 HTML 양식 원본 tml_ccp_pkg_NNN
--
-- 파일번호: 111
-- 이전번호: 110
-- 개발자: 박승우
-- 일자: 2026-08-20
-- 코멘트:
--   1) 저장은 tbl_tml_ccp_pkg_ver. 예시는 tml_ccp_pkg_000 (한계기준·주기·방법·개선조치 5항목)
--   2) 새 html_sys 카탈로그는 두지 않는다. copy SP가 usr tbl_template 를 직접 INSERT 한다
--   3) 작성 화면은 후속. 기본 주기 D. 문서주기 좌측은 tml_ccp_pkg_001+ 만
--
-- ============================================================
...
-- 1. DDL
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS tbl_tml_ccp_pkg_ver (
    ...
    CONSTRAINT ux_tbl_tml_ccp_pkg_ver UNIQUE (co_cd, tmpl_cd, ver_no),
    CONSTRAINT ck_tbl_tml_ccp_pkg_ver_no CHECK (ver_no >= 1),
    CONSTRAINT ck_tbl_tml_ccp_pkg_ver_apply CHECK (apply_yn IN ('Y', 'N')),
    CONSTRAINT ck_tbl_tml_ccp_pkg_ver_use CHECK (use_yn IN ('Y', 'N'))
);
CREATE UNIQUE INDEX IF NOT EXISTS ux_tbl_tml_ccp_pkg_ver_apply
    ON tbl_tml_ccp_pkg_ver (co_cd, tmpl_cd) WHERE apply_yn = 'Y';
CREATE UNIQUE INDEX IF NOT EXISTS ux_tbl_tml_ccp_pkg_ver_cd
    ON tbl_tml_ccp_pkg_ver (co_cd, tmpl_cd, ver_cd) WHERE use_yn = 'Y';
COMMENT ON TABLE tbl_tml_ccp_pkg_ver IS 'CCP-1B 포장 모니터링일지 자사 양식 버전 — 예시는 tml_ccp_pkg_000 가상';

CREATE TABLE IF NOT EXISTS tbl_tml_ccp_pkg_ver_item (
    idx        bigint       GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    co_cd      varchar(10)  NOT NULL,
    tmpl_cd    varchar(40)  NOT NULL,
    ver_no     int          NOT NULL,
    ...
    upd_id     varchar(20)  NULL,
    upd_dt     timestamp    NULL,
    CONSTRAINT ux_tbl_tml_ccp_pkg_ver_item UNIQUE (co_cd, tmpl_cd, ver_no, item_cd)
);
COMMENT ON TABLE tbl_tml_ccp_pkg_ver_item IS 'CCP-1B 포장 모니터링일지 자사 양식 항목 — 한계기준·주기·방법·개선조치';

-- ------------------------------------------------------------
-- 2. 시드 항목 — tml_ccp_pkg_000. 긴 문구는 item_nm
-- ------------------------------------------------------------
INSERT INTO tbl_check_item (
    tmpl_cd, item_cd, grp_cd, grp_nm, item_nm, input_type, unit_nm, cycle_nm, sort_no, ins_id
) VALUES
    ('tml_ccp_pkg_000', 'limit-temp', 'limit', '한계기준',
     '15℃ 이하',
     'text', NULL, '작업장 온도', 1, 'system'),
    ('tml_ccp_pkg_000', 'limit-time', 'limit', '한계기준',
     '2분 50초 이상 3분 30초 이하',
     'text', NULL, '포장 완료 시까지 소진시간', 2, 'system'),
    ('tml_ccp_pkg_000', 'cycle', 'cycle', '주기',
     '작업 시작 전 / 작업 중 1시간 마다 / 작업 종료 후',
     'text', NULL, NULL, 3, 'system'),
    ('tml_ccp_pkg_000', 'method', 'method', '방법',
     $m$◌ 작업장 온도 : 모니터링 담당자는 유니트쿨러에 부착된 판넬에 표시되는 온도를 확인하여 일지에 기록한다.
◌ 포장 완료 시까지 소진시간 : 모니터링 담당자는 타이머를 이용하여 포장 시작 시점(해피 직후 계량 시작)부터 포장 완료 시점(내포장 완료)까지 실시한 후 타이머 세팅값을 일지에 기록한다.
※ 유니트쿨러 온도계/타이머는 연 1회 검·교정을 실시한다.$m$,
     'text', NULL, NULL, 4, 'system'),
    ('tml_ccp_pkg_000', 'corrective', 'ca', '개선조치 방법',
     $c$○ 작업장 온도 초과 시
...
-- ------------------------------------------------------------
-- 3. 화면·권한·메뉴 — HTML양식 원본 밑, 검증점검 다음
-- ------------------------------------------------------------
...
SELECT r.co_cd, r.usrgrp_cd, 'ccp-pkg-template',
       'Y', 'Y', 'Y',
...
SELECT c.co_cd, 'menu-ccp-pkg-template', COALESCE(s.scrn_nm, 'ccp-pkg-template'),
       'menu-master-html', 'ccp-pkg-template',
       COALESCE((
...
DROP FUNCTION IF EXISTS sp_tbl_tml_ccp_pkg_ver_item_r_000(varchar, varchar, int);
CREATE FUNCTION sp_tbl_tml_ccp_pkg_ver_item_r_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
    -- p_tmpl_cd: tml_ccp_pkg_000 이면 시드
    p_tmpl_cd varchar,
    -- p_ver_no: 0=표준
    p_ver_no int
)
...
DROP FUNCTION IF EXISTS sp_tbl_tml_ccp_pkg_ver_copy_c_000(varchar, varchar, int, varchar, varchar, varchar);
CREATE FUNCTION sp_tbl_tml_ccp_pkg_ver_copy_c_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
    -- p_tmpl_cd: 호환. 시드는 tml_ccp_pkg_000 고정
    p_tmpl_cd varchar,
...
BEGIN
    ...
    SELECT COALESCE(MAX(substring(t.tmpl_cd from '[0-9]{3}$')::int), 0) INTO v_n
      FROM tbl_template t
     WHERE t.tmpl_cd ~ '^tml_ccp_pkg_[0-9]{3}$' AND t.tmpl_cd <> 'tml_ccp_pkg_000';
    LOOP
...
DROP PROCEDURE IF EXISTS sp_tbl_tml_ccp_pkg_ver_item_u_000(varchar, varchar, int, jsonb, varchar);
CREATE PROCEDURE sp_tbl_tml_ccp_pkg_ver_item_u_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
...
DROP FUNCTION IF EXISTS sp_tbl_tml_ccp_pkg_ver_delete_blocker_r_000(varchar, varchar, int);
CREATE FUNCTION sp_tbl_tml_ccp_pkg_ver_delete_blocker_r_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
...
DROP PROCEDURE IF EXISTS sp_tbl_tml_ccp_pkg_ver_d_000(varchar, varchar, int, varchar);
CREATE PROCEDURE sp_tbl_tml_ccp_pkg_ver_d_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
    -- p_tmpl_cd: 자사 양식코드
    p_tmpl_cd varchar,
...
DROP PROCEDURE IF EXISTS sp_tbl_tml_ccp_pkg_ver_apply_u_000(varchar, varchar, int, varchar);
CREATE PROCEDURE sp_tbl_tml_ccp_pkg_ver_apply_u_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
    -- p_tmpl_cd: 자사 양식코드
    p_tmpl_cd varchar,
...
-- ------------------------------------------------------------
-- 5. 문서주기 — tml_ccp_pkg_001+ 포함. 예시 000 숨김
-- ------------------------------------------------------------
...
COMMENT ON FUNCTION sp_schedule_cycle_management_form_r_000(varchar, varchar, varchar, varchar) IS
  '문서주기관리 좌측 — html_hyg_prc_001+ · tml_ccp_chk_001+ · tml_ccp_pkg_001+ · html_sys_002~005·007~010·012 · hwp. 예시 000 숨김';
