...
BEGIN
    SELECT h.idx,d.status INTO v_hdr_idx,v_status FROM tbl_document d JOIN tbl_ccp_metal_monitor h ON h.doc_idx=d.idx AND h.co_cd=d.co_cd WHERE d.co_cd=p_co_cd AND d.idx=p_doc_idx AND d.tmpl_cd='CCP_METAL' AND d.del_yn='N';
...
BEGIN
    ...
    ELSIF p_tmpl_cd='VERIFY_PLAN' THEN
...
BEGIN
    IF p_tmpl_cd NOT IN ('CCP_VERIFY','VERIFY_PLAN') THEN RAISE EXCEPTION '지원하지 않는 CCP 양식입니다.' USING ERRCODE='45000'; END IF;
    IF COALESCE(p_base_dt,'')='' OR (p_tmpl_cd='CCP_VERIFY' AND length(p_base_dt)<>8) OR (p_tmpl_cd='VERIFY_PLAN' AND length(p_base_dt)<>4) THEN RAISE EXCEPTION '기준일 형식이 올바르지 않습니다.' USING ERRCODE='45000'; END IF;
    ...
    IF p_doc_idx IS NULL OR p_doc_idx=0 THEN
      INSERT INTO tbl_document(co_cd,tmpl_cd,doc_kind,doc_no,base_dt,title,status,appr_line_cd,writer_id,write_dt,ver_no,retention_until,del_yn,ins_id) VALUES(p_co_cd,p_tmpl_cd,'DB',sp_tbl_doc_no_gen_c_000(p_co_cd,p_tmpl_cd,p_base_dt||CASE WHEN p_tmpl_cd='VERIFY_PLAN' THEN '0101' ELSE '' END),p_base_dt||CASE WHEN p_tmpl_cd='VERIFY_PLAN' THEN '0101' ELSE '' END,v_name||' ('||p_base_dt||')','WRK',v_appr,p_id,now(),1,to_char((to_date(p_base_dt||CASE WHEN p_tmpl_cd='VERIFY_PLAN' THEN '0101' ELSE '' END,'YYYYMMDD')+(COALESCE(v_retain,24)||' months')::interval)::date,'YYYYMMDD'),'N',p_id) RETURNING idx INTO v_doc_idx;
      IF p_tmpl_cd='CCP_VERIFY' THEN INSERT INTO tbl_ccp_verify_check(co_cd,doc_idx,base_dt,checker_id,checker_nm,ins_id) VALUES(p_co_cd,v_doc_idx,p_base_dt,NULLIF(p_checker_id,''),NULLIF(p_checker_nm,''),p_id) RETURNING idx INTO v_hdr_idx;
    ...
    ELSE
      SELECT d.idx,d.status,CASE WHEN p_tmpl_cd='CCP_VERIFY' THEN v.idx ELSE p.idx END INTO v_doc_idx,v_status,v_hdr_idx FROM tbl_document d LEFT JOIN tbl_ccp_verify_check v ON v.doc_idx=d.idx AND v.co_cd=d.co_cd AND p_tmpl_cd='CCP_VERIFY' LEFT JOIN tbl_verify_plan p ON p.doc_idx=d.idx AND p.co_cd=d.co_cd AND p_tmpl_cd='VERIFY_PLAN' WHERE d.co_cd=p_co_cd AND d.idx=p_doc_idx AND d.tmpl_cd=p_tmpl_cd AND d.del_yn='N';
      ...
      UPDATE tbl_document SET base_dt=p_base_dt||CASE WHEN p_tmpl_cd='VERIFY_PLAN' THEN '0101' ELSE '' END,title=v_name||' ('||p_base_dt||')',upd_id=p_id,upd_dt=now() WHERE co_cd=p_co_cd AND idx=v_doc_idx;
      IF p_tmpl_cd='CCP_VERIFY' THEN UPDATE tbl_ccp_verify_check SET base_dt=p_base_dt,checker_id=NULLIF(p_checker_id,''),checker_nm=NULLIF(p_checker_nm,''),upd_id=p_id,upd_dt=now() WHERE co_cd=p_co_cd AND idx=v_hdr_idx; DELETE FROM tbl_ccp_verify_item WHERE co_cd=p_co_cd AND hdr_idx=v_hdr_idx;
...
DECLARE v_hdr_idx bigint; v_status varchar(4);
BEGIN
    SELECT d.status, CASE WHEN p_tmpl_cd='CCP_VERIFY' THEN v.idx ELSE p.idx END INTO v_status,v_hdr_idx
      FROM tbl_document d
