-- ============================================================
--  migrate 73 — 메뉴 관리 화면 SP를 화면명 규약으로 신규 작성 (v2)
--
--  개발자: 박승우
--  일자: 2026-08-12
--  코멘트:
...
--    5) 생성 전용 — 구 sp_tbl_menu_mgmt_*·sp_tbl_menu_nav_* DROP은 80에서 수행
-- ============================================================

SET search_path TO sasshaccp;

-- ------------------------------------------------------------
-- 1. sp_menu_management_r_000 — 관리화면 메뉴 전체 목록
--    로그인 사용자의 조회권한으로 필터하지 않는다(관리자 화면)
--    좌측 트리가 전체 집합을 필요로 하므로 FE는 헤더 파라미터를 공백으로 호출하고 화면에서 다시 거른다
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_menu_management_r_000(varchar, varchar, varchar, varchar);
CREATE FUNCTION sp_menu_management_r_000(
...
$$;
COMMENT ON FUNCTION sp_menu_management_r_000(varchar, varchar, varchar, varchar) IS
  '메뉴 관리 목록 — 권한 필터 없음. 헤더 메뉴코드·명·사용여부 LIKE, sort_no 순';

-- ------------------------------------------------------------
-- 2. sp_menu_nav_r_000 — 사이드바 메뉴 트리 (권한 반영)
--    로그인 직후 1회 호출한다. 조회권한(read_yn)이 없는 화면은 아예 내려보내지 않는다
--    관리화면과 달리 use_yn='Y'만, 그리고 leaf는 권한이 있어야 통과한다
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_menu_nav_r_000(varchar, varchar);
CREATE FUNCTION sp_menu_nav_r_000(
    -- p_co_cd: JWT 회사코드 — 테넌트 범위. 필수 등가 조건
    p_co_cd     varchar,
...
RETURNS TABLE(
    ...
    SELECT m.idx, m.menu_cd, m.menu_nm, m.h_menu_cd, m.scrn_cd, s.module_cd, m.sort_no,
     ...
     ORDER BY m.sort_no, m.menu_cd;
$$;
COMMENT ON FUNCTION sp_menu_nav_r_000(varchar, varchar) IS
  '사이드바 메뉴 트리 — 사용중 메뉴만, leaf는 조회권한 Y일 때만. sort_no 순';

-- ------------------------------------------------------------
-- 3. sp_menu_management_c_000 — 메뉴 저장
--    메뉴코드·계층·화면코드는 화면에서 편집 불가라 실제로는 메뉴명·사용여부만 바뀐다
--    사용여부를 N으로 내리면 자손 메뉴도 전부 N으로 내려 트리에 고아 노드가 남지 않게 한다
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE sp_menu_management_c_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd    varchar,
...
BEGIN
    ...
    END IF;
END$$;
COMMENT ON PROCEDURE sp_menu_management_c_000(varchar, bigint, varchar, varchar, varchar, varchar, int, varchar, varchar) IS
  '메뉴 저장 — use_yn=N이면 자손 전체 N 전파. 메뉴코드는 화면에서 수정 불가';

-- ------------------------------------------------------------
-- 4. sp_menu_management_delete_blocker_r_000 — 삭제 참조 검증
--    하위 메뉴가 남으면 트리가 끊기므로 차단한다
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_menu_management_delete_blocker_r_000(varchar, bigint[]);
CREATE FUNCTION sp_menu_management_delete_blocker_r_000(
    -- p_co_cd: JWT 회사코드
    p_co_cd varchar,
    -- p_idxs: 삭제 대상 대리키 배열
    p_idxs  bigint[]
...
RETURNS TABLE(ref_key varchar, target varchar) LANGUAGE sql AS $$
    SELECT m.menu_cd::varchar AS ref_key,
     ...
     LIMIT 1;
$$;
COMMENT ON FUNCTION sp_menu_management_delete_blocker_r_000(varchar, bigint[]) IS
  '메뉴 삭제 차단 — 하위 메뉴 보유 시 불가. 위반 첫 건만 반환';

-- ------------------------------------------------------------
-- 5. sp_menu_management_d_000 — 메뉴 삭제
--    blocker와 같은 조건을 삭제 직전에 다시 본다
-- ------------------------------------------------------------
CREATE OR REPLACE PROCEDURE sp_menu_management_d_000(
...
END$$;
COMMENT ON PROCEDURE sp_menu_management_d_000(varchar, bigint) IS
