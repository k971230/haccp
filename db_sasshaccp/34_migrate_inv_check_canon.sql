-- ============================================================
-- 34 — 재고 점검 양식코드 INV → INV_CHECK 통일
--
-- 개발자: 박승우
-- 일자: 2026-08-06
-- 코멘트:
--   1) 시드·BizOps SP·Controller는 INV_CHECK를 쓰는데 구 시드 tmpl_cd=INV 가 남아 저장이 실패했다
--   2) 기존 테넌트의 템플릿·화면·사용양식·채번·문서를 INV_CHECK로 옮긴다
--   3) 이미 INV_CHECK만 있으면 환경에서는 no-op에 가깝게 동작한다
-- ============================================================

SET search_path TO sasshaccp;

-- 표준양식 — INV만 있고 INV_CHECK가 없을 때 행을 복제해 코드를 바꾼다
...
SELECT 'INV_CHECK', tmpl_nm, mng_no, doc_kind, category_cd, scrn_cd,
       default_cycle_cd, default_retention_month, impl_yn, sort_no, 'system', now()
  FROM tbl_template
 WHERE tmpl_cd = 'INV'
   AND NOT EXISTS (SELECT 1 FROM tbl_template WHERE tmpl_cd = 'INV_CHECK')
ON CONFLICT (tmpl_cd) DO NOTHING;

-- INV_CHECK가 이미 있으면 경우에도 화면·명칭을 inventory-check와 맞춘다
UPDATE tbl_template
   SET tmpl_nm = COALESCE(tmpl_nm, '입·출고 및 재고 점검표'),
       ...
       upd_id = 'system',
       upd_dt = now()
 WHERE tmpl_cd = 'INV_CHECK';

UPDATE tbl_screen
   SET tmpl_cd = 'INV_CHECK',
       upd_id = 'system',
       upd_dt = now()
 WHERE scrn_cd = 'inventory-check'
   AND COALESCE(tmpl_cd, '') IN ('INV', 'INV_CHECK', '');
