-- ============================================================
-- 119 — 중분류 menu-master-form 을 menu-master-hwp 로 맞춤
--
-- 파일번호: 119
-- 이전번호: 118
-- 개발자: 박승우
-- 일자: 2026-08-21
-- 코멘트:
--   1) URL 중칸은 /docs/hwp/ 가 정본이다. DB 중분류 접미만 form → hwp 로 맞춘다
--   2) h_menu_cd 는 varchar 코드라 FK 가 없다. 부모 menu_cd 먼저, 자식 h_menu_cd 나중
--   3) tbl_screen · leaf menu_cd · 화면권한은 건드리지 않는다
--
...
UPDATE tbl_menu SET menu_cd = 'menu-master-hwp', upd_id = 'system', upd_dt = now()
 WHERE menu_cd = 'menu-master-form';

-- 2) leaf 부모 참조만 이동
UPDATE tbl_menu SET h_menu_cd = 'menu-master-hwp', upd_id = 'system', upd_dt = now()
 WHERE h_menu_cd = 'menu-master-form';

-- 3) sort 인코딩 SP — 13·21 과 동일. 118 정의의 form 을 hwp 로 교체한다
...
BEGIN
    UPDATE tbl_menu m
      ...
      FROM (VALUES
        ...
        ('menu-master-sch', 4100), ('menu-master-hwp', 4200),
    ...
    WITH mid AS (
        SELECT * FROM (VALUES
            ...
            ('menu-master-sch', 4, 1), ('menu-master-hwp', 4, 2),
