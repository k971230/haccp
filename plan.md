...
> 확정한 결함을 고치는 계획이다.
> **이 문서는 2026-09-03 계획 스냅샷이다.** 고치는 것은 다른 세션이 한다.
> `10_*.sql`·`11_*.sql`·`12_*.sql` 은 적용한 뒤 저장소에서 지웠다. 정본 시드는 7본.
> 결함 목록과 근거는 당시 코드검토 기록 ·
...
## 4-2. 진행 상황 (2026-09-07)

브랜치 `fix/ddl-comment-doc-kind`. 1~9 커밋은 `fix/hwp-body-and-send-guard` · **PULL REQUEST #99**.
...
| 3 | K9 | **끝** `2678f2f` |
| 4 | K5-b·K6·K10·K16·K17-b·K21 | **끝** `fd96969` — `01_sp.sql` + `10_ca_guard_and_numbering.sql` |
| 5 | K11·K12·K26·K27·K28 | **끝** `3edeb95` — `useLatestOnly` 훅 하나로 다섯 화면 |
| 6 | C7 | **끝** `d3e8dba` |
| 7 | K7·K23 | **끝** `5ddc30a` — 세 자리가 한 뿌리였다 |
| 8 | K1 | **끝** `d617b3e` |
| 9 | K17-a 그리드 maxLength | **끝** `591215a` — 19칸 + `audit_grid_maxlength.sh` |
| 9b | K3·C4·K13·K14·K15·K29·K30·K31 | **끝** — 워킹트리 (앞 세션, 검수 반영까지) |
| — | **K32** 결재선 업서트 | **끝** — 시험 DB `10_` 적용, E2E 3 passed |
| 7b | K22·K25·K24 | **끝** — 검수 2회, 결함 3건 반영 후 결함 없음 |
| 10 | K18·K2·K19·K20 | **끝** — `11_` 시험 DB 적용, 검수 결함 없음 |
| P3 | K4·C5·C6 | **끝** — 색인 1건 반영 후 결함 없음 |
| 2차·3차 | 인가 닫힘·401 구분·초안 seen·apply-all 재실행·Pref·가족 포트 | **점검 끝** — 시험 DB seen SP 적용, HWP seenUpdDt 누락 1건 고침 |
| A+B | 포장·가열 표 분리 + 전면 DTO | **점검 끝** — 12_split 멱등 1건·색인 재생성 |

**PULL REQUEST:** #99(1~5) 머지됨 · #100(6~8) 열려 있음.

**K12·K26·K27 은 한 뿌리였다** — 「응답을 상태에 쓰는데 순번 가드가 없다」.
다섯 곳에 복사하는 대신 `hooks/useLatestOnly.ts` 를 만들고 그걸 쓴다.
`HtmlFormDraftPage.detailSeq` 가 같은 것을 손으로 하고 있으나 E2E 가 지키고 있어 안 건드렸다 —
고칠 일이 생기면 그때 모은다.
...
### 검수가 두 번째로 잡은 것 — 잠금을 빼면 표시가 따라 죽는다

K27 에서 `asyncAct.run(..., key)` 를 빼면서 **그 key 의 `isBusy` 를 보는 자리를 안 봤다.**

| 자리 | 결과 |
|---|---|
| `RoleManagementPage.tsx`:447 `isBusy("tree")` | 영영 `false` — 「불러오는 중…」이 안 뜨고 빈 트리가 곧장 그려진다 |
| `CommonCodePage.tsx`:364·397 `isBusy("detail")` | 두 그리드 로딩 표시가 안 뜬다 — 느린 응답이 「항목 없음」으로 보인다 |

로컬 `loading` 상태로 옮겼다. **끄는 것은 최신 적재일 때만** — 낡은 응답이 새 적재의 표시를 끄면 안 된다.

**주석·README 도 같이 낡았다** — `UserManagementPage.tsx`:283 과
`pages/sys/code/user/README.md` 가 「`lockYn` 은 이 화면이 다루지 않는다」라고 적고 있었다.
K11 이 그걸 뒤집었으므로 둘 다 고쳤다.

**교훈:** 잠금·플래그를 **빼는** 수정은 그것을 **읽는** 자리를 전부 grep 해야 한다.
넣을 때는 컴파일이 잡아 주지만 뺄 때는 아무도 안 잡는다.

### 순서를 밟으며 드러난 것

**K7 을 「HWP 덮어쓰기 한 자리」로 좁게 잡은 것이 틀렸다.** 전수 라운드(K23)가 두 자리를 더 찾았고,
고쳐 보니 **셋이 한 뿌리**였다 — 되돌림 단위가 다른 둘을 한 트랜잭션에 섞은 것.
`deleteAfterCommit` 하나로 모았다. 처음부터 뿌리를 봤으면 K7·K23 을 나눌 일이 없었다.

**K1 은 주석이 이미 참말을 하고 있었다** — 「완료 상태 검사는 SP 가 다시 한다 (Double Check)」.
코드만 그렇지 않았다. **주석과 코드가 어긋난 자리는 대개 코드가 틀린 쪽**이다.
...
- **주석은 고치는 커밋에 같이 넣는다.** 밀도는 `.cursor/rules/05-handoff-comments.mdc`
...
### K32. 결재선만 업무키로 업서트한다 — 신규 행에 기존 코드를 치면 그 결재선이 통째로 갈린다 — P1

**층: SP.** 9번(K17-a)을 하다 나왔다. 내 변경이 만든 것이 아니라 **원래 있던 것**이다.

형제 셋은 신규 행의 업무키 중복을 막는다.

| SP | 신규 행에 기존 코드를 치면 |
|---|---|
| `sp_common_code_management_c_000`:657 | `RAISE '이미 등록된 코드입니다'` |
| `sp_department_management_c_000`:834 | `RAISE '이미 등록된 부서코드입니다'` |
| `sp_role_management_c_000`:1683 | `RAISE '이미 등록된 권한그룹코드입니다'` |
| **`sp_tbl_approval_line_c_000`**:2188 | **`ON CONFLICT (co_cd, appr_line_cd) DO UPDATE`** — 막지 않는다 |

그리고 바로 다음 줄(:2195)이 `DELETE FROM tbl_approval_line_step` 이다.
그래서 **이름을 덮고 단계를 통째로 지운 뒤 새 행의 단계로 갈아 끼운다.**

**깨지는 경로:** 결재선관리에서 행추가 → 결재선코드에 이미 있는 `DEFAULT` 를 치고 저장.
`DEFAULT` 의 이름이 바뀌고 승인 단계가 사라진다. 그 결재선을 쓰는 모든 양식의
**결재가 누구에게 가는지가 바뀐다.** 되돌릴 자료가 없다.

`newOnly: ["apprLineCd"]`(`ApprovalLineManagementRule.ts`:51)는 **저장된 뒤에** 잠그는 것이라
신규 행에서 남의 코드를 치는 것은 못 막는다.

| 파일 | 무엇 |
|---|---|
| `01_sp.sql` `sp_tbl_approval_line_c_000` | 신규(`p_idx` 없음 또는 화면이 신규로 보낸 경우)면 형제와 같은 중복 검사. 기존 행 수정은 지금대로 |

**업서트를 그냥 없애면 안 된다** — 같은 SP 가 기존 결재선 수정에도 쓰인다.
가르는 기준을 먼저 정해야 한다(화면이 신규 여부를 보내는가, `p_idx` 가 있는가).
**형제 셋이 무엇으로 가르는지 보고 같은 기준을 쓴다.**

**K17-a 와의 관계:** 상한을 걸면 25자가 20자로 잘려 남의 코드와 겹칠 길이 하나 는다.
다만 그리드는 **입력 시점에** 자르므로 사용자가 잘린 값을 보고 저장한다 — 조용하지 않다.
그래도 이 SP 가 막아야 할 것을 안 막는다는 사실은 그대로다.
