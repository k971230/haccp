# HACCP SaaS — 타 AI 리뷰용 덤프 (`완성.md`)

> **살아 있는 정본이 아니다.** 2026-08-11 시점의 타 AI 리뷰 덤프. 이후 계층 URL(`tabRoute`)·패키지 `com.haccp.docs`·탭 우클릭 메뉴는 이 파일에 없다.  
> 지금 읽을 곳: 루트 [`README.md`](README.md) · [`docs/15_HACCP_FE_BE_통합_상세스펙.md`](docs/15_HACCP_FE_BE_통합_상세스펙.md) · [`docs/23_PIPELINE.md`](docs/23_PIPELINE.md) · [`docs/20_배포_런북.md`](docs/20_배포_런북.md).  
> 이식한 사실: 운영 Path `/haccp/`, 포트 4173/7070, GHCR `ghcr.io/k971230/haccp-*`, Jenkins `DEPLOY_DIR=/home/ubuntu/haccp` — 런북·루트 README에 이미 있다. 공개 IP는 이 덤프에만 있으므로 정본으로 옮기지 않는다(환경값).
...
| [`docs/20_배포_런북.md`](docs/20_배포_런북.md) | 배포 정본 |
| `docs/` · `docs/` | 앱 상세 스펙 |

문서 정본은 루트 `docs/1_`~`21_`이다. FE/BE 하위 `docs/`는 제거되었다.
...
|------|------------------|--------------|-------------|
| STEP 00 | 루트 `docs/20_배포_런북.md` 킥오프(브랜치·커밋·PR·시크릿) | Plan→실행, feature 브랜치 | 배포 런북 뼈대 |
...
CRUD·메뉴 전수: `docs/07_*.md`, `08_*.md` · BE `docs/06_*.md` · SQL `db_sasshaccp/`.
...
| #15 | MERGED | docs 1_~21_ 모음·ports·migrate CI 제거 등 |
...
| 항목 | 설명 |
|------|------|
| 문서 drift | 문서 정본은 `docs/1_`~`21_` (구 FE/BE docs 제거) |
